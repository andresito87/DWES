...
{
    $ubicaciones = Ubicaciones::listarUbicaciones($pdo);
    $html = '<table border="1"> 
    <thead><tr><th>Id</th><th>Nombre</th><th>Descripción</th><th>Días</th><th>Acciones</th></tr></thead>';
    foreach ($ubicaciones as $ubicacion) {
        $html .= '<tr><td>' . $ubicacion->getId() . '</td>' . '<td>' . $ubicacion->getNombre() . '</td><td>' . $ubicacion->getDescripcion() . '</td><td>' . $ubicacion->getDias() . '</td><td><button onclick="' . rq()->call('enviarDatosUbicacionParaModificar', $ubicacion->getId()) . '">Modificar</button>' . '<button onclick="' . rq()->call('eliminarUbicacion', $ubicacion->getId()) . '">Eliminar</button></td></tr>';
    }
    $html .= '</table>';
...
function crearUbicacion(int|string $id, array $datos)
{
    ...
    $ubicacion->setNombre($datos['nombre']);
    $ubicacion->setDescripcion($datos['descripcion']);
    $ubicacion->setDias(implode(',', $datos['dias']));
    $resultado = $ubicacion->guardar($pdo);
...
function enviarDatosUbicacionParaModificar(int $id)
{
    ...
    $ubicacion = Ubicacion::rescatar($pdo, $id);

    if ($ubicacion instanceof Ubicacion) {
        $response->assign('id', 'value', $ubicacion->getId());
        $response->assign('nombre', 'value', $ubicacion->getNombre());
        $response->assign('descripcion', 'value', $ubicacion->getDescripcion());
        foreach (Ubicacion::DIAS as $dia) {
            if (in_array($dia, explode(',', $ubicacion->getDias()))) {
...
function modificarUbicacion(int $id, array $datos)
{
    ...
    // Rescatamos la ubicación a modificar y convetimos el id a entero, jaxon lo pasa como string.
    $ubicacion = Ubicacion::rescatar($pdo, intval($id));
...
function eliminarUbicacion(int $id)
{
    ...
    $resultado = Ubicacion::borrar($pdo, $id);
    if ($resultado > 0) {
        cargarListadoUbicaciones($pdo, $response);
    } else {
        logMessage($response, DB::getLastError());
