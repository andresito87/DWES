...
class Taller extends Modelo
{
    ...
    public function setNombre(string $nombre):bool {
        if (empty($nombre) || strlen($nombre)>255) return false;
        $this->nombre = $nombre;
        return true;
    }

    public function setDescripcion(string $descripcion):bool {
        if (empty($descripcion) || strlen($descripcion)>65535) return false;
        $this->descripcion = $descripcion;
        return true;
    }

    public function setUbicacion(string $ubicacion) {
        if (empty($ubicacion) || strlen($ubicacion)>255) return false;
    ...
    public function setHora_inicio(string $hora_inicio) {
        if (!preg_match('/^[0-5][0-9]:[0-9]{2}$/',$hora_inicio)) return false;
        $this->hora_inicio = $hora_inicio;
        return true;
    }

    public function setHora_fin(string $hora_fin):bool {
        if (!preg_match('/^[0-5][0-9]:[0-9]{2}$/',$hora_fin)) return false;
        $this->hora_fin = $hora_fin;
        return true;
    }

    public function setCupo_maximo(int $cupo_maximo):bool {
        if ($cupo_maximo<5 || $cupo_maximo>30) return false;
    ...
    public static function rescatar(PDO $pdo, int $id): Taller | bool {
        ...
        {   
            $stmt=$pdo->prepare($SQL);
            $stmt->bindValue('id',$id); 
            if ($stmt->execute()) $resultado=$stmt->fetchObject(__CLASS__); 
            if ($resultado instanceof Taller)
            {
                $resultado->setId($resultado->id);
                unset($resultado->id);
            }
        } catch (Exception $ex) {
            var_dump($ex);
